...
export default function TrendingKenyaShoplink() {
  ...
  const [liveMeta, setLiveMeta] = useState(null); // { date, capturedAt } from the daily snapshot
  ...
  async function fetchLiveStories() {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/trending/today`);
      if (!res.ok) throw new Error(`Backend responded ${res.status}`);
      const json = await res.json();
      const stories = Array.isArray(json.stories) ? json.stories : [];
      if (stories.length === 0) throw new Error("Today's snapshot isn't captured yet");
      setLiveStories(stories);
      setLiveMeta({ date: json.date, capturedAt: json.capturedAt });
      setFeedStatus("live");
    } catch (err) {
    ...
    <div style={{ minHeight: "100vh", background: COLORS.ink, color: COLORS.chalk, fontFamily: F_BODY }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", flexWrap: "wrap", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <Badge color={feedStatus === "live" ? COLORS.lime : feedStatus === "loading" ? COLORS.gold : COLORS.mutedDim}>
              {feedStatus === "live" && liveMeta
                ? `● Today's trends captured ${new Date(liveMeta.capturedAt).toLocaleTimeString()}`
                : feedStatus === "loading" ? "Connecting…" : "Curated backup data"}
            </Badge>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "22px 20px 60px" }}>
        ...
        <div style={{ marginTop: 28, padding: 14, borderTop: `1px solid ${COLORS.border}`, fontSize: 11, color: COLORS.mutedDim, display: "flex", gap: 6, alignItems: "flex-start" }}>
          ...
          trending-backend instance (see its README) to pull today's automatically-captured trending set instead — that backend polls real
          sources continuously and takes one official daily snapshot each morning (Africa/Nairobi time), so "today's trending" updates itself
          without anyone running this by hand. This page falls back to the curated set automatically if the live feed is unreachable.
        </div>
      </div>
    </div>
  );
